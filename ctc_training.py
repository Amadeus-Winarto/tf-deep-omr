...
step_size = primus.getTrainingSize // params['batch_size']
...
for epoch in range(max_epochs):
    ...
    if epoch % args.frequency == 0:
        # VALIDATION
        print ('Loss value at epoch ' + str(epoch) + ':' + str(mean(loss_value)))
        print ('Validating...')
        ...
        for step in range(val_step):
            validation_batch = primus.nextBatch(params, mode = 'Train')
            ...
            for i in range(len(str_predictions)):
                val_ed += ctc_utils.edit_distance(str_predictions[i], validation_batch['targets'][val_idx+i])
